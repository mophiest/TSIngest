...
function RecordingsPage({ data, refresh, notify }: PageProps) {
  ...
  return <>
    ...
    <section className="panel recordings-panel">{items.length ? <div className="recording-list">{items.map(item => { const ts = fileOf(item, 'ts'), mp4 = fileOf(item, 'mp4'), mp4State = mp4 ? 'ready' : item.mp4Job?.status; return <article className="recording-item" key={item.id}>
      <button className="recording-main" onClick={() => setSelected(item)}><div className={`recording-icon ${item.status}`}><Film /></div><div className="recording-name"><strong>{item.streamName}</strong><span>{formatDate(item.startedAt || item.requestedAt, true)} · <span className="mono">{shortId(item.id)}</span></span></div><StatusBadge status={item.status} /><div className="recording-stat"><span>时长</span><b>{formatDuration(item.progressMs || ts?.durationMs || 0)}</b></div><div className="recording-stat"><span>TS 大小</span><b>{formatBytes(ts?.sizeBytes || item.progressSize)}</b></div><div className="file-chips"><span className={ts ? 'ready' : ''}>TS</span><span className={mp4ChipClass(mp4State)}>MP4{mp4State && mp4State !== 'ready' ? ` · ${statusLabel(mp4State)}` : ''}</span></div></button>
      <div className="recording-actions">{item.status === 'recording' || item.status === 'waiting_input' ? <button className="button stop compact" disabled={busy === item.id} onClick={() => run(item.id, () => api.stopRecording(item.id), '停止命令已发送')}><Square />停止</button> : ts && !mp4 && item.mp4Job?.status !== 'queued' && item.mp4Job?.status !== 'running' ? <button className="button secondary compact" disabled={busy === item.id} onClick={() => run(item.id, () => api.generateMp4(item.id), item.mp4Job?.status === 'failed' ? 'MP4重试任务已加入队列' : 'MP4任务已加入队列')}><Film />{item.mp4Job?.status === 'failed' ? '重试 MP4' : '生成 MP4'}</button> : null}{canClearRecording(item) && <button className="icon-button danger-ghost" title={item.status === 'failed' ? '清除告警' : '删除空记录'} disabled={busy === item.id} onClick={() => setConfirmClear(item)}><Trash2 size={18} /></button>}{mp4 && <button className="icon-button" title="预览MP4" onClick={() => setPreview(item)}><Eye size={18} /></button>}{mp4 && <a className="icon-button" title="下载MP4" href={`/api/v1/recordings/${item.id}/files/mp4?download=1`}><Download size={18} /></a>}{ts && <a className="icon-button" title="下载TS" href={`/api/v1/recordings/${item.id}/files/ts?download=1`}><Download size={18} /></a>}</div>
      {item.mp4Job && !mp4 && <div className={`job-progress ${item.mp4Job.status}`} title={item.mp4Job.error || statusLabel(item.mp4Job.status)}><span style={{ width: `${item.mp4Job.status === 'queued' ? 2 : Math.max(2, item.mp4Job.progress)}%` }} /><b>{mp4JobLabel(item.mp4Job.status, item.mp4Job.progress)}</b></div>}
...
function RecordingDrawer({ recording, onClose, onPreview, onDelete }: { recording: Recording; onClose: () => void; onPreview: () => void; onDelete: (kind: 'ts' | 'mp4') => void }) {
  ...
  return <><button className="drawer-scrim" onClick={onClose} /><aside className="drawer"><div className="modal-head"><div><span className="eyebrow">RECORDING DETAIL</span><h2>{recording.streamName}</h2></div><button className="icon-button" onClick={onClose}><X /></button></div><StatusBadge status={recording.status} /><div className="detail-grid"><Detail label="任务 ID" value={recording.id} mono /><Detail label="开始时间" value={formatDate(recording.startedAt || recording.requestedAt,true)} /><Detail label="结束时间" value={recording.endedAt ? formatDate(recording.endedAt,true) : '—'} /><Detail label="结束原因" value={reasonLabel(recording.endReason)} /><Detail label="录制时长" value={formatDuration(recording.progressMs || ts?.durationMs || 0)} /><Detail label="自动 MP4" value={recording.autoMp4 ? '已启用' : '未启用'} /></div>{video && <section className="detail-section"><h3>视频</h3><div className="codec-card"><Film /><div><strong>{video.codec.toUpperCase()} · {video.profile || 'Unknown profile'}</strong><span>{video.width} × {video.height}</span></div></div></section>}<section className="detail-section"><h3>音轨 <span>{audios.length}</span></h3>{audios.map((audio,index)=><div className="audio-row" key={index}><span>A{index+1}</span><div><strong>{audio.codec.toUpperCase()}</strong><small>{audio.channels} 声道 · {audio.sampleRate || '—'} Hz {audio.language ? `· ${audio.language}` : ''}</small></div></div>)}</section>{recording.mp4Job && <section className="detail-section"><h3>MP4 转换</h3><div className={`mp4-status-card ${recording.mp4Job.status}`}><StatusBadge status={recording.mp4Job.status} /><div><strong>{mp4JobLabel(recording.mp4Job.status, recording.mp4Job.progress)}</strong><div className="mp4-status-bar"><span style={{width:`${recording.mp4Job.status==='queued'?2:Math.max(2,recording.mp4Job.progress)}%`}} /></div>{recording.mp4Job.error && <p>{recording.mp4Job.error}</p>}</div></div></section>}<section className="detail-section"><h3>文件</h3>{[ts,mp4].filter(Boolean).map(file=><div className="file-row" key={file!.id}><span className="file-kind">{file!.kind.toUpperCase()}</span><div><strong>{file!.name}</strong><small>{formatBytes(file!.sizeBytes)}</small></div><div className="file-row-actions">{file!.kind==='mp4'&&<button className="icon-button" onClick={onPreview}><Eye /></button>}<a className="icon-button" href={`/api/v1/recordings/${recording.id}/files/${file!.kind}?download=1`}><Download /></a><button className="icon-button danger-ghost" onClick={()=>onDelete(file!.kind)}><Trash2 /></button></div></div>)}</section>{recording.error&&<div className="error-box"><AlertTriangle /><div><strong>任务异常</strong><p>{recording.error}</p></div></div>}</aside></>
}
...
function mp4ChipClass(status:string|undefined){return status==='ready'?'ready':status==='queued'||status==='running'?'working':status==='failed'?'failed':''}
function mp4JobLabel(status:string,progress:number){if(status==='queued')return 'MP4 排队中';if(status==='running')return `MP4 转换中 ${progress.toFixed(0)}%`;if(status==='failed')return 'MP4 转换失败';if(status==='ready')return 'MP4 已完成';return statusLabel(status)}
